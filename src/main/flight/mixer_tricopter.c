...
void triServoMixer(int16_t PIDoutput)
{
    ...
    if (triflightConfig()->tri_servo_feedback != TRI_SERVO_FB_VIRTUAL)
    {
        ...
        tailServoADC = pt1FilterApply4(&feedbackFilter, adcGetChannel(tailServoADCChannel), 70, dT);
	}
...
static void checkMotorAcceleration(void)
{
    ...
    {
        pidResetErrorAccumulatorsAxis(FD_YAW);
    }

    // Set the expected axis error based on tail motor acceleration and configured gain
    pidSetExpectedGyroError(FD_YAW, acceleration * triflightConfig()->tri_motor_acc_yaw_correction / 10);
}
...
static void tailTuneModeThrustTorque(thrustTorque_t *pTT, const bool isThrottleHigh)
{
    ...
    case TT_ACTIVE:
        if (isThrottleHigh &&
            ...
            (fabsf(gyro.gyroADCf[FD_YAW]) <= 4.0f)) // deg/s
        {
